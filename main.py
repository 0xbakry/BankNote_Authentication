...
class DecisionTree():
    ...
    def build_tree(self, banknote_datatset, current_depth=0):
        ...
        if num_of_samples >= self.min_samples_split and current_depth <= self.max_depth:
            ...
            if (best_node_to_split["info_gain"] > 0):
                ...
                return Node(best_node_to_split["feature_index"], best_node_to_split["considered"], left_subtree, right_subtree,
    ...
    def information_gain(self, parent, left_child, right_child, mode="entropy"):
        ...
        if mode == "gaining":
            result = self.gainig_index(parent) - (left_weight * self.gainig_index(left_child) + right_weight * self.gainig_index(right_child))
        else:
            result = self.entropy(parent) - (left_weight * self.entropy(left_child) + right_weight * self.entropy(right_child))
    ...
    def print_tree(self, tree=None, space=" "):
        ...
        if tree.value is not None: print(tree.value)
        else:
...
train_features, test_features, train_labels, test_labels = train_test_split(dataset_features, dataset_labels, test_size=0.25)

classifier = DecisionTree(min_samples_split=3, max_depth=3)
classifier.fit(train_features, train_labels)
classifier.print_tree()
classifier.print_size()
